...
- Use `message_react` tool to send the reaction to the user message.
...
- If `final.is_lightweight` is true, you must choose to only provide an emoji by using `message_react` tool instead of sending a text message.
- If `final.is_lightweight` is false, you do NOT use `message_react` tool.
