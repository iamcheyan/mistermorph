...
このテストは既定で `codex-acp` を探します。別の adapter を使う場合は `MISTERMORPH_ACP_CODEX_COMMAND` と `MISTERMORPH_ACP_CODEX_ARGS` を設定してください。
...
分離した `mistermorph-acp-adapters` を使う場合は、`command` と `args` をその別 checkout かインストール結果へ向けてください。
